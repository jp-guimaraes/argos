...
///
/// `cancel` is owned by the caller (backlog #35): `main.rs` creates it and
/// hands the same one to a background thread watching stdin for a
/// cancellation signal, so a `SIGINT` the unprivileged parent catches can
/// reach the write loop below mid-copy, not just at process start.
pub fn execute(
    plan: &WritePlan,
